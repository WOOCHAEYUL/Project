...
find_plate_model = YOLO("/home/pi/workspace/parking_v1/model/find_plate_240123.pt")
plate_char_model = YOLO("/home/pi/workspace/parking_v1/model/plate_char_240118.pt")
...
def detect_plate_v8(img_path) :
    ...
    frame = cv2.imread("/home/pi/workspace/parking_v1/"+img_path, cv2.IMREAD_COLOR)
    #현재(ver.01) 카메라 180도 회전되어 달려있음 -> 프로그램으로 회전시켜줌
    #frame = cv2.rotate(frame1, cv2.ROTATE_180)
    #image 폴더에 raw.jpg로 저장
    cv2.imwrite('/home/pi/workspace/parking_v1/image/raw.jpg',frame)
...
def detect_char_v8(frame, plate_type):
    ...
    if obj_list :
        if plate_type == 0: # 1 line plate
        ...
        elif plate_type == 1: # 2 line plate        
            ...
            top_line.sort(key=lambda x: x[2]) # 각 라인에서 y 좌표를 기준으로 정렬
            bottom_line.sort(key=lambda x: x[2])

            top_line.sort(key=lambda x: x[1]) # 각 라인에서 x 좌표를 기준으로 정렬
