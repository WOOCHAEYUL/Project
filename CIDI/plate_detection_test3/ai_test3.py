...
def extract_gt_from_filename(filename):
    ...
    return [(plate, '차량') for plate in gt_list]
...
def compare_results_with_gt(result_list, gt_list):
    ...
    # GT 분리
    gt_all = [plate for plate in gt_list]    
    # Detected 분리
    detected_all = [plate for plate in result_list]
    ...
    # TP와 FP 계산
    detected_pass = []
    for detected in detected_all:
        if detected in gt_all:
            tp += 1
            gt_all.remove(detected)  # GT에서 해당 항목 제거
        else:
...
def state(img_file, result_folder):
    global roi_pts, roi_set, scale_factor
    global gt_list
    ...
    if img is not None:
        ...
        gt_list = extract_gt_from_filename(img_file)
        
        detected_plate_numbers = [(result[1], '차량') for result in (obj_list or [])]
        ...
        if crop_images and obj_list:
            ...
            tp, fp, fn = compare_results_with_gt(detected_plate_numbers, gt_list)
            accuracy = tp / (tp + fn + fp) if (tp + fn + fp) else 0
            
            logging.info(f"Plates - (TP: {tp}, FP: {fp}, FN: {fn})")
            result_and_save_image(img, obj_list, save_path, accuracy, result_folder)
            position(obj_list, img_width)
        else:
            fn = len(gt_list)
            tp, fp = 0, 0
            logging.info(f"Plates - (TP: 0, FP: 0, FN: {fn})")
            logging.info("GT 값은 있지만 검출된 번호판이 없습니다.")
            result_and_save_image(img, [], save_path, 0, result_folder)
...
def result_and_save_image(img, position_list, save_path, accuracy, result_folder):
    plate_number_list = []
    result_details = []
    ...
    excel_path = os.path.join(result_folder, "plate_result.xlsx")
    ...
    sheet.append([img_name, gt_summary, result_summary, accuracy])    
