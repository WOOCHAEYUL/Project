...
def position(position_list, img_width):
    if len(position_list) == 1:
        ...
        #logging.info(f"차량 번호: {position_list[0][1]}, 위치: {position}")
        ...
        for position_info in position_list:
            ...
            #logging.info(f"차량 번호: {position_info[1]}, 위치: {position}")
...
def extract_gt_from_filename(filename, disabled_plates):
    base_name = os.path.splitext(os.path.basename(filename))[0]
    parts = base_name.split('_')
    plate_pattern = re.compile(r'\d{2,3}[가-힣]\d{4}')
    gt_list = [part for part in parts if plate_pattern.match(part)]
    return [(plate, '장애인차량' if plate in disabled_plates else '불법주차차량') for plate in gt_list]
...
def compare_results_with_gt(result_list, gt_list):
    ...
    #logging.info("&&gt_list", gt_list)
    
    # 장애인, 불법주차차량 GT 분리
    gt_illegal = [plate for plate in gt_list if plate[1] == '불법주차차량']
    gt_disabled = [plate for plate in gt_list if plate[1] == '장애인차량']
    
    # 장애인, 불법주차차량 Detected 분리
    detected_illegal = [plate for plate in result_list if plate[1] == '불법주차차량']
    detected_disabled = [plate for plate in result_list if plate[1] == '장애인차량']

    # 각각의 GT, Detected 출력
    #logging.info("**gt_illegal", gt_illegal)
    #logging.info("**gt_disabled", gt_disabled)
    #logging.info("**detected_illegal", detected_illegal)
    #logging.info("**detected_disabled", detected_disabled)
    
    # FN 계산: 전체 GT와 Detected의 갯수를 비교
    if len(gt_list) > len(result_list):
        for gt_plate in gt_list:
            if gt_plate not in result_list:
                if gt_plate[1] == '불법주차차량':
                    fn_illegal += 1
                elif gt_plate[1] == '장애인차량':
                    fn_disabled += 1
    ...
    for detected in detected_illegal:
        if detected in gt_illegal:
            tp_illegal += 1
            gt_illegal.remove(detected)  # GT에서 해당 항목 제거
        else:
            detected_pass.append(detected)

    for detected in detected_disabled:
        if detected in gt_disabled:
            tp_disabled += 1
            gt_disabled.remove(detected)  # GT에서 해당 항목 제거
        else:
            detected_pass.append(detected)

    # 남아 있는 GT 항목 기준으로 FP 계산
    ...
    return tp_illegal, fp_illegal, fn_illegal, tp_disabled, fp_disabled, fn_disabled

def state(img_file, result_folder, disabled_plates):
    global roi_pts, roi_set, scale_factor
    global gt_list
    ...
    if img is not None:
        ...
        gt_list = extract_gt_from_filename(img_file, disabled_plates)
        
        detected_plate_numbers = [(result[1], '장애인차량' if result[1] in disabled_plates else '불법주차차량') for result in (obj_list or [])]
        ...
        if crop_images and obj_list:
            ...
            #    save_crop(crop, save_path, f"crop_{i}")
            tp_illegal, fp_illegal, fn_illegal, tp_disabled, fp_disabled, fn_disabled = compare_results_with_gt(detected_plate_numbers, gt_list)
            accuracy_illegal = tp_illegal / (tp_illegal + fn_illegal + fp_illegal) if (tp_illegal + fn_illegal + fp_illegal) else 0
            accuracy_disabled = tp_disabled / (tp_disabled + fn_disabled + fp_disabled) if (tp_disabled + fn_disabled + fp_disabled) else 0
            
            logging.info(f"Illegal Plates - (TP: {tp_illegal}, FP: {fp_illegal}, FN: {fn_illegal})")
            logging.info(f"Disabled Plates - (TP: {tp_disabled}, FP: {fp_disabled}, FN: {fn_disabled})")
            result_and_save_image(img, obj_list, save_path, accuracy_illegal, accuracy_disabled, disabled_plates)
            position(obj_list, img_width)
        else:
            fn_illegal = sum(1 for gt_plate in gt_list if gt_plate[1] == '불법주차차량')
            fn_disabled = sum(1 for gt_plate in gt_list if gt_plate[1] == '장애인차량')
            tp_illegal, fp_illegal, tp_disabled, fp_disabled = 0, 0, 0, 0
            logging.info(f"Illegal Plates - (TP: 0, FP: 0, FN: {fn_illegal})")
            logging.info(f"Disabled Plates - (TP: 0, FP: 0, FN: {fn_disabled})")
            logging.info("GT 값은 있지만 검출된 번호판이 없습니다.")
            result_and_save_image(img, [], save_path, 0, 0, disabled_plates)
...
"""def result_and_save_image(img, position_list, save_path, accuracy_illegal=None, accuracy_disabled=None, disabled_plates=[]):
    plate_number_list = []
    result_details = []
    accuracies = []
    ...
    for pos_info in position_list:
        ...
        if plate_number in disabled_plates:
            img = myPutText(img, "장애인 차량", (pos_info[2], pos_info[5] + 10), FONT_SIZE, (0, 0, 255))
            result_details.append(f"{plate_number}:장애인차량")
        else:
